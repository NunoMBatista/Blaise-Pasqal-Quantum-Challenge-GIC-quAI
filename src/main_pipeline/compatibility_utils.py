...
"""
    behaviour: 
        Check if a graph is compatible with AnalogDevice and print issues
    input: 
...
def check_analog_device_compatibility(graph_data):
    ...
    if hasattr(graph_data, 'pos') and graph_data.pos is not None:
        max_radius = getattr(AnalogDevice, 'max_distance_from_center', 35.0)
    ...
    if hasattr(graph_data, 'pos') and graph_data.pos is not None and graph_data.pos.shape[0] > 1:
        pos = graph_data.pos.numpy()
        min_dist = AnalogDevice.min_atom_distance
...
"""
    behaviour: 
        Create a minimal working graph compatible with AnalogDevice
    input: 
...
"""
def debug_graph_compatibility(graph, device=AnalogDevice):
    ...
    if is_compatible:
        print("Graph appears to be compatible with AnalogDevice")
    else:
        print("Graph is NOT compatible with AnalogDevice. Issues found:")
        for issue in issues:
            print(f" - {issue}")
...
"""
    behaviour: 
        make a graph compatible with the AnalogDevice
    input: 
        graph_data - a torch_geometric.data.data object
    output: 
        modified graph_data compatible with AnalogDevice
"""
def make_compatible_with_analog_device(graph_data):
    ...
    # 2. AnalogDevice requires undirected graphs - ensure bidirectional edges
    edge_index = graph_data.edge_index.numpy()
    edge_set = set()
    ...
    max_radius = getattr(pl.AnalogDevice, 'max_distance_from_center', 35.0)  # Default to 35μm
    pos = graph_data.pos.numpy()
